...
\begin{tabular}{|c|c|c|c|}
    \hline
    Spider fusion & Identity removal & Hadamard identity & $\pi$-commutation \\
    \tikzfig{rule-sf} & \tikzfig{rule-id} &  \tikzfig{rule-had-id} & \tikzfig{rule-pi-com} \\ \hline
    State copy & Color Change & Bialgebra &  Hopf \\
    \tikzfig{rule-state-copy} & \tikzfig{rule-colour-change} & \tikzfig{rule-bialg} & \tikzfig{rule-hopf} \\  \hline
\end{tabular}
}
\end{table}
...
% Expanding Toffoli into its basic gates leads to 
...
% These optimisations  slightly reduced the number of Clifford gates in the diagram. 
%More 
...
%As such there are a lot of Hadamard gates and Z-spiders that could cancel-out or change color. 
...
This fully-simplified circuit now surpasses the original circuit in both the total amount of gates and T-count.%, but not the one obtained with the simple optimisations above. 
